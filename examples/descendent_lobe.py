...
def compute_lobe_axes(lobe, slope, input, lobe_dimensions):
    ...
    aspect_ratio = min(
        input.max_aspect_ratio, 1.0 + input.aspect_ratio_coeff * slope_norm
    )
...
pfy.flowycpp.mr_lava_loba.perturb_lobe_angle(
    parent_lobe, np.linalg.norm(slope_parent_center), simulation.input, gen
)

# Perimeter of the parent lobe, rasterized so you can see it
perimeter = np.array(
    [
        parent_lobe.point_at_angle(phi)
...
pfy.flowycpp.mr_lava_loba.perturb_lobe_angle(
    descendent_lobe, np.linalg.norm(slope_parent_center), simulation.input, gen
)  # This should have set the azimuthal angle

pfy.flowycpp.mr_lava_loba.add_inertial_contribution(
    descendent_lobe, parent_lobe, np.linalg.norm(slope_parent_center), simulation.input
)
...
print(
    "Azimuthal angle of the descendent in pi ",
    descendent_lobe.get_azimuthal_angle() / np.pi,
)
...
new_lobe_center = compute_descendent_lobe_position(
    descendent_lobe, parent_lobe, final_budding_point
)
descendent_lobe.center = new_lobe_center
print("new lobe center ", new_lobe_center)
perimeter_descendent = np.array(
    [descendent_lobe.point_at_angle(phi) for phi in np.linspace(0, 2 * np.pi, 30)]
)
...
plt.plot(
    perimeter_descendent[:, 0], perimeter_descendent[:, 1], color="red"
)  # descendent lobe
# Budding points
